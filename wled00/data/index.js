...
function starSet(x){
	...
	switch (taps[star]) {
		case 1:
			...
			d.getElementById(btnId).innerHTML = "✋";
			starRGB = startRGB;
			createProbJSON(star);
			liveJSON = probJSON;
			updateApi(0);
			...
			break;
		case 2:
			d.getElementById(btnId).style.backgroundColor = "rgb(" + holdRGB + ")";
			d.getElementById(btnId).innerHTML = "😀";
			starRGB = holdRGB;
			createProbJSON(star);
			liveJSON = probJSON;
...
function getRandomEmoji(i){
	...
	document.getElementById(emojiInputID).value =
	document.getElementById(emojiInputID).defaultValue = emoji;
...
<div class="c">Save to ID <input class="noslide" id="p${i}id" type="number" oninput="checkUsed(${i})" max=250 min=1 value=${(i>0)?i:getLowestUnusedP()}></div>
<div class="c">
	<button class="btn btn-i btn-p btn-save" onclick="saveProb(${i})"><i class="icons btn-icon">&#xe390;</i>${(i > 0) ? "Save changes" : "Save"}</button>
	${(i > 0) ? '<button class="btn btn-i btn-p btn-delete" onclick="delP(' + i + ')"><i class="icons btn-icon">&#xe037;</i></button>' :
		'<button class="btn btn-p" onclick="resetProbUtil()">Cancel</button>'}
</div>
<div class="pwarn ${(i>0)?"bp":""} c" id="p${i}warn">
